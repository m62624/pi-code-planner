...
- `migrate-layout.ts` → `migrateLayout` runs once per session start (best-effort, idempotent): moves legacy flat `extensionDir/plans/<planId>` into the owning `projects/<projectId>/plans/<planId>` (attributed via `project.json`), and drops the stale `skills/bundled/` system-skill dir. Never destroys a source on a destination collision; leaves orphan plans and the legacy global user-skill pool untouched.
