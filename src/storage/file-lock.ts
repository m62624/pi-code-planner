...
// message concurrently, so a batch of writes to the same record (e.g. the model
// authoring all tasks at once, or a tool call racing the idle watchdog) would
// otherwise each read the same base, mutate their own copy, and let the last
// writer win — silently dropping the rest. Every record store routes its
// `update*` helper through this so "no lost update" is one invariant in one
// place.
//
