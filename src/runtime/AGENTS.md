...
- **Compaction is token-driven, not a state-machine ceremony.** The window-management compact steps (`compact_discovery`/`spec`/`planning`/`task` and `compact_finalize`) were removed from `PLANNER_STAGE_STEPS`: the proactive `turn_end` monitor in `index.ts` (`projectPlannerContextBudget`) compacts on any step when the output-aware budget says so, so the model no longer pays a `finish_step` round-trip to walk a boundary that mostly no-ops below the floor. The one surviving compact step is `compact_before_doubt` — a deliberate confidence reset before the doubt audit, so `isPlannerCompactEnabled` returns true for it unconditionally and `index.ts` forces it (`force` flag) past the token floor. A legacy `state.json` parked at a removed step is remapped forward by `normalizePlanState` (`storage/state-store.ts`). The `compactBoundaries` (`compact.stage`/`compact.task`) setting is now inert — it gated only the removed boundaries.
