...
const WRITABLE_CONTRACT_BASENAMES = ["AGENTS.md", "AGENTS.MD"] as const;
...
export function parsePlannerContractMarkdown(
			...
			diagnostics: [
				{
					...
					message:
						"Contract file exists but has no pi-code-planner managed block. Read it as user-authored guidance; update through planner_contract_upsert if planner facts need to be durable.",
					path,
				},
			],
		};
	...
	if (starts !== 1 || ends !== 1) {
		diagnostics.push({
			...
			message: `Expected exactly one managed block, found start=${starts}, end=${ends}.`,
			path,
		});
	}
	...
	if (lines[0]?.trim() !== MANAGED_TITLE) {
		diagnostics.push({
			severity: "error",
			code: "missing_title",
			message: `Managed block must start with ${MANAGED_TITLE}.`,
			path,
		});
	}
		...
		if (heading) {
			if (!isKnownSectionHeading(heading)) {
				diagnostics.push({
					...
					message: `Unknown contract heading: ${heading}.`,
					path,
				});
				current = null;
				continue;
	...
	for (const heading of SECTION_HEADINGS) {
		if (!sections.has(heading)) {
			diagnostics.push({
				...
				message: `Missing contract heading: ${heading}.`,
				path,
			});
		}
	}
...
> {
	...
	while (queue.length > 0 && remaining > 0) {
		...
		for (const name of CONTRACT_BASENAMES) {
		...
		for (const entry of entries.sort()) {
			if (
				...
				(CONTRACT_BASENAMES as readonly string[]).includes(entry)
			) {
				continue;
			}
