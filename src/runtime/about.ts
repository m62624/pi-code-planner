...
export function buildPlannerAboutReport(input: {
	...
	return [
		...
		"- planner_elenchus_check is available at the discovery scan, planning/consistency_check, execution/write_tdd_plan, execution/contract_check, finalize/doubt_review, and recovery/repair_or_resume. It fits a web of interacting constraints (exactly-one-owner, mutually-exclusive states, gate/branch coverage, access matrices, dependency ordering); resolution=not_applicable with a reason is the terminal escape so the flow never deadlocks.",
		"- The gate is hard only on CONFLICT: planner_finish_step refuses to finish a step whose latest check proved a contradiction — apply the drop/flip repair its CORE/RETRACT names and re-run (or re-run as not_applicable if the check modeled the wrong thing). WARNING/UNDERDETERMINED never block, but CONSISTENT is the target before advancing.",
		"- A bundled premise-template library (plan-consistency, tdd-gate, branch-contract, doubt-review, discovery-gaps, recovery-state, ship-gate) is synced into each plan's elenchus/templates/ dir. A program IMPORTs one (IMPORT \"templates/<name>.vrf\"), states facts into the template's domain, and binds the template's VAR ports through the tool's values parameter. Project overrides live in .pi/pi-code-planner/vrf/<name>.vrf.",
